...
;(function () {
  ...
  function amCopy(text, btn) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        .writeText(text)
        .then(done)
        .catch((e) => Logger('WARN', 'Не удалось скопировать в буфер', e))
    } else {
  ...
  function createSingleLogEl(entry) {
    ...
    if (entry.details) {
      ...
      header.onclick = (e) => {
        ...
        el.querySelector('.am-log-expand').style.transform = isHidden
          ? 'rotate(180deg)'
          : 'rotate(0deg)'
      }
    }
  ...
  function appendLogEntry(entry) {
    ...
    const canGroup =
      activeLogFilter === 'ALL' && !activeSearchQuery && ['API', 'DB', 'QUEUE'].includes(entry.type)
    ...
    if (canGroup && lastChild) {
      if (
        ...
        lastChild.classList.contains(`type-${entry.type.toLowerCase()}`)
      ) {
        ...
        header.onclick = () => {
          ...
          groupEl.querySelector('.am-log-expand').style.transform = isHidden
            ? 'rotate(180deg)'
            : 'rotate(0deg)'
        }
  ...
  function openLoggerModal() {
      ...
      <div class="am-logger-modal" style="position:relative;">
        <div class="am-logger-header">
          ...
          <div class="am-logger-filters">
            <button
              class="am-log-filter ${activeLogFilter === 'ALL' ? 'active' : ''}"
              data-filter="ALL"
            >
              ALL
            </button>
            <button
              class="am-log-filter ${activeLogFilter === 'INFO' ? 'active' : ''}"
              data-filter="INFO"
            >
              INFO
            </button>
            <button
              class="am-log-filter ${activeLogFilter === 'WARN' ? 'active' : ''}"
              data-filter="WARN"
            >
              WARN
            </button>
            <button
              class="am-log-filter ${activeLogFilter === 'API' ? 'active' : ''}"
              data-filter="API"
            >
              API
            </button>
            <button
              class="am-log-filter ${activeLogFilter === 'DB' ? 'active' : ''}"
              data-filter="DB"
            >
              DB
            </button>
            <button
              class="am-log-filter ${activeLogFilter === 'QUEUE' ? 'active' : ''}"
              data-filter="QUEUE"
            >
              QUEUE
            </button>
            <button
              class="am-log-filter ${activeLogFilter === 'ERROR' ? 'active' : ''}"
              data-filter="ERROR"
            >
              ERROR
            </button>
    ...
    document.getElementById('am-log-close').onclick = () => {
      overlay.remove()
      isLoggerOpen = false
    }
    document.getElementById('am-log-clear').onclick = () => {
      scriptLogs = []
    ...
    document.getElementById('am-log-state').onclick = async () => {
      ...
      const state = {
        ...
        rateLimits: {
          alRateLimitPause:
            alRateLimitPause > Date.now() ? new Date(alRateLimitPause).toLocaleTimeString() : 'OK',
          shikiRateLimitPause:
            shikiRateLimitPause > Date.now()
              ? new Date(shikiRateLimitPause).toLocaleTimeString()
              : 'OK',
          anime365: {
            rateLimitPause:
              anime365RateLimitPause > Date.now()
                ? new Date(anime365RateLimitPause).toLocaleTimeString()
                : 'OK',
   ...
   */
  function cmpGetIgnore() {
    try {
      return new Set(JSON.parse(GM_getValue('CMP_IGNORE', '[]')))
    } catch (e) {
  ...
  async function cmpFetchAniListList(userName, type) {
    ...
    const lists =
      (res && res.data && res.data.MediaListCollection && res.data.MediaListCollection.lists) || []
    const map = new Map()
  ...
  async function cmpFetchAniListFavs(userName, kind) {
    ...
    while (true) {
      ...
      const fav =
        ...
        res.data &&
        res.data.User &&
        res.data.User.favourites &&
        res.data.User.favourites[kind]
      if (!fav) break
      for (const n of fav.nodes || []) {
        if (n.idMal)
          map.set(n.idMal, (n.title && (n.title.romaji || n.title.english)) || 'MAL#' + n.idMal)
      }
  ...
  async function cmpFetchShikiFavs(userId) {
    ...
    const toMap = (arr) => {
      const m = new Map()
      ;(arr || []).forEach((x) => {
        if (x && x.id) m.set(x.id, x.russian || x.name || 'MAL#' + x.id)
      })
      return m
    }
  ...
  async function cmpFetchAniListFavPeople(userName, kind) {
    ...
    while (true) {
      ...
      if (!fav) break
      for (const n of fav.nodes || [])
        arr.push({ name: (n.name && n.name.full) || '', native: (n.name && n.name.native) || '' })
      if (!fav.pageInfo || !fav.pageInfo.hasNextPage) break
      page++
      await new Promise((r) => setTimeout(r, 700))
    }
    ...
    for (const id of ids) {
      ...
      if (s.status !== a.status)
        out.status.push({
          id,
          title,
          shiki: cmpStatusLabel(s.status),
          al: cmpStatusLabel(a.status),
        })
      ...
      if (pDiff)
        out.progress.push({ id, title, shiki: cmpFmtProg(s, type), al: cmpFmtProg(a, type) })
  ...
  function cmpRenderSummary(label, sh, al) {
    const rows = CMP_STATUS_ORDER.map(
      (s) =>
  ...
  function cmpRenderNameFavs(label, diff) {
    const sec = (l, arr) => {
      ...
      const items = arr
        .slice(0, 500)
        .map(
          (x) => `<div class="amk-diffrow"><span class="amk-name">${cmpEsc(x.title)}</span></div>`,
        )
        .join('')
      const more =
        arr.length > 500
          ? `<div style="opacity:.6;padding:6px;">…ещё ${arr.length - 500}</div>`
          : ''
  ...
  function cmpRender(resultEl) {
    ...
    resultEl.innerHTML = html`<div style="display:flex;gap:20px;flex-wrap:wrap;">
      ...
      <div style="opacity:.5;font-size:11px;margin-top:14px;line-height:1.5;">
        «В списке только на одной площадке» — не ошибка синка, а различие каталогов/списков.
        «Связано с уже отслеживаемым» — вероятно деление на сезоны или сиквелы (по связям AniList).
        Крестик ✕ — скрыть строку (игнор, запоминается). Даты не сравниваются. Оценки нормализованы
        к 10-балльной. Сопоставление по MAL id.
      </div>`

    resultEl.querySelectorAll('.cmp-ignore').forEach(
      (el) =>
        (el.onclick = () => {
          cmpAddIgnore(el.dataset.id)
          cmpRender(resultEl)
        }),
    )
    resultEl.querySelectorAll('.cmp-unignore').forEach(
      (el) =>
        (el.onclick = () => {
          cmpRemoveIgnore(el.dataset.id)
          cmpRender(resultEl)
        }),
    )
  }
  ...
  async function cmpRunScan(shikiLogin, alName, statusEl, resultEl, deepCheck) {
    ...
    try {
      ...
      const [shA, alA] = [
        await cmpFetchShikiList(shikiId, 'anime'),
        await cmpFetchAniListList(alName, 'ANIME'),
      ]
      setStatus('Загружаю списки (манга)...')
      const [shM, alM] = [
        await cmpFetchShikiList(shikiId, 'manga'),
        await cmpFetchAniListList(alName, 'MANGA'),
      ]
      ...
      GM_xmlhttpRequest({
        ...
        onload: (res) => {
          if (res.status === 200) {
            ...
            setTimeout(
              () => resolve(anilistQuery(query, variables, useAuth)),
    ...
    for (const domain of ANIME365_DOMAINS) {
      try {
          ...
          GM_xmlhttpRequest({
            ...
            onload: (r) => {
              ...
              // 403/503 + Cloudflare (520–524) — soft-block, не «нет данных».
                ...
                resolve({ __block: true, status: r.status })
              else reject(r.status)
            },
            onerror: reject,
            ontimeout: reject,
        ...
        if (res.__block) {
          ...
          Logger(
            'WARN',
            `anime365 недоступен: HTTP ${res.status} (${domain}). Сбой ${anime365FailStreak}/${ANIME365_FAIL_LIMIT}, бэкофф 15с.`,
          )
        ...
        Logger(
          'WARN',
          `Сбой запроса к зеркалу anime365: ${domain} (${e}). Сбой ${anime365FailStreak}/${ANIME365_FAIL_LIMIT}.`,
        )
        if (anime365FailStreak >= ANIME365_FAIL_LIMIT) {
          anime365Disabled = true
      ...
      if (src === 'shikimori') {
        ...
        if (shiki.data && shiki.data.russian) {
          return {
            ...
            description: shiki.data.description,
  ...
  function amTokens(str) {
    return amNormRomaji(str).split(' ').filter(Boolean)
  }
  function amNormNative(str) {
    ...
    for (const domain of SHIKI_DOMAINS) {
      try {
        ...
        const searchUrls = [
          ...
          ...(nameParts.length > 1
            ? [
                `https://${domain}/api/${endpointStr}/search?search=${encodeURIComponent(reversedName)}`,
              ]
            : []),
        ...
        if (item && item.id) {
          let rDetails = await new Promise((resolve) =>
            GM_xmlhttpRequest({
              method: 'GET',
          ...
          if (rDetails.status === 200) {
            try {
              detailsRes = JSON.parse(rDetails.responseText)
            } catch (e) {
          ...
          if (targetMalIds && targetMalIds.length && itemScore < 90 && detailsRes) {
            let candMal = []
            if (Array.isArray(detailsRes.animes))
              detailsRes.animes.forEach((a) => a && a.id && candMal.push(a.id))
            if (Array.isArray(detailsRes.mangas))
              detailsRes.mangas.forEach((a) => a && a.id && candMal.push(a.id))
            if (Array.isArray(detailsRes.works))
              detailsRes.works.forEach(
                (w) => w && w.anime && w.anime.id && candMal.push(w.anime.id),
              )
            if (Array.isArray(detailsRes.roles))
              detailsRes.roles.forEach((rr) =>
                (rr.animes || []).forEach((a) => a && a.id && candMal.push(a.id)),
              )
          ...
          if (detailsRes) {
            return {
              ...
              data: {
                ...
                russian: detailsRes.russian || item.russian,
            ...
            return {
              status: 200,
              data: { id: item.id, russian: item.russian, description: null, domain },
            }
          }
        }
      } catch (e) {
  ...
  async function resolveShikiPersonByMedia(personData, type) {
    ...
    let mediaRefs = mediaNodes
      .filter((m) => m.idMal)
      .map((m) => ({ id: m.idMal, kind: m.type === 'MANGA' ? 'mangas' : 'animes' }))
  ...
  function initExporter() {
    ...
    async function fetchShikiHistoryDates(userId, btn) {
      ...
      for (const id in datesMap) {
        const starts = datesMap[id].starts
        const ends = datesMap[id].ends
    ...
    async function syncShikiToAlList(shikiItems, type, alUser, historyDates, btn) {
      ...
      for (const item of valids) {
        ...
        if (historyDates && historyDates[item.target_id]) {
          if (historyDates[item.target_id].start)
            startedAt = makeFuzzyDate(historyDates[item.target_id].start)
          if (historyDates[item.target_id].end)
            completedAt = makeFuzzyDate(historyDates[item.target_id].end)
        }
        ...
        if (ex) {
          ...
          if (notes !== undefined)
            isSame = isSame && (ex.notes ? ex.notes.trim() : undefined) === notes
    ...
    async function openExportModal(btn) {
      ...
      const dUser =
        urlPath.length > 1 && !['animes', 'mangas', 'forum'].includes(urlPath[1]) ? urlPath[1] : ''
      ...
      document.getElementById('se-start').onclick = async () => {
        ...
        if (!exportAnime && !exportManga && !exportFavs)
          return alert('Выберите опции для экспорта!')
  ...
  function initTranslator() {
    ...
    function cleanShikiBB(text, url, sourceName = 'Shikimori') {
      ...
      return (
        bbHtml +
        `<br><br><small style="opacity:0.75;font-size:0.85em;">Описание предоставлено <a href="${safeUrl}" target="_blank" rel="noopener noreferrer" style="color:#3dbbee; font-weight:bold;">${escapeHTML(sourceName)}</a></small>`
      )
    }
    ...
    function translateAdvanced(text) {
      ...
      if (cleanText.includes(' · ')) {
        return cleanText
          ...
          .map((p) => {
            return (
              (Object.prototype.hasOwnProperty.call(dictionary, p.trim())
                ? dictionary[p.trim()]
                : null) ||
              translateAdvanced(p.trim()) ||
              p.trim()
            )
          })
          .join(' · ')
      ...
      if ((match = cleanText.match(rxRole))) {
        let roleTr =
          (Object.prototype.hasOwnProperty.call(dictionary, match[1].trim())
            ? dictionary[match[1].trim()]
            : null) || match[1].trim()
        let episodes = match[2]
          .trim()
      ...
      if ((match = cleanText.match(rxTimeComplex))) {
        const p1 = translateAdvanced(match[1])
        const p2 = translateAdvanced(match[2])
        if (p1 && p2) return `${p1} ${p2}`
      }
      if ((match = cleanText.match(rxHeight)))
        return `${match[1].trim()} см${match[2] ? ` (${match[2]})` : ''}`
      if ((match = cleanText.match(rxLiked))) return `${match[1]} из ${match[2]} оценили этот отзыв`
      if ((match = cleanText.match(rxDateFull)))
        return `${match[2]} ${monthsFull[match[1]]} ${match[3]} г.`
      if ((match = cleanText.match(rxBday)))
        return match[2].length > 2
          ? `${monthsFull[match[1]]} ${match[2]} г.`
          : `${match[2]} ${monthsFull[match[1]]}`
      ...
      if ((match = cleanText.match(rxAct))) {
        ...
        const actRu = {
          watched: isRange ? 'Просмотрены' : 'Просмотрена',
          rewatched: isRange ? 'Пересмотрены' : 'Пересмотрена',
          read: isRange ? 'Прочитаны' : 'Прочитана',
          reread: isRange ? 'Перечитаны' : 'Перечитана',
        }
        const typeRu = {
          episode: isRange ? 'серии' : 'серия',
          chapter: isRange ? 'главы' : 'глава',
        }
      ...
      if ((match = cleanText.match(rxRecent)))
        return `${match[1]} недавно ${match[2].toLowerCase() === 'watched' ? 'смотрели' : 'читали'}`
      if ((match = cleanText.match(rxReviewBy))) return `отзыв от ${match[1]}`
      if ((match = cleanText.match(rxDayDate)))
        return `${days[match[1]]}, ${match[3]} ${monthsFull[match[2]]} ${match[4]} г.`
      ...
      if ((match = cleanText.match(rxListAdded))) {
        const title =
          (Object.prototype.hasOwnProperty.call(dictionary, match[1])
            ? dictionary[match[1]]
            : null) || match[1]
        const listsMap = {
          completed: 'Просмотрено',
    ...
    function translateNode(node) {
      ...
      if (
        ...
        !['SCRIPT', 'STYLE', 'NOSCRIPT', 'SVG'].includes(node.tagName)
      ) {
    ...
    function processTooltip(tooltipNode) {
      ...
      let targetLink = deepest.closest(
      ...
      if (targetLink) {
        ...
        if (matchMed && settings.translateTitles) {
          targetId = matchMed[2]
          targetType = 'MED2'
        } else if (matchChar && settings.translateCharacters) {
          targetId = matchChar[1]
          targetType = 'CHR2'
          extra = matchChar[2]
        } else if (matchStaff && settings.translateStaff) {
          targetId = matchStaff[1]
          targetType = 'STF3'
          extra = matchStaff[2]
        }
      ...
      debounceTimer = setTimeout(() => {
        ...
        document
          ...
          .forEach((link) => {
            ...
            if (
              isMedia &&
              (link.classList.contains('relation-title') ||
                link.closest('.relations') ||
                link.closest('.role'))
            )
              return
        ...
        if (settings.translateTitles) {
          ...
          if (m) {
            ...
            if (desc && (!desc.querySelector('.ru-desc') || desc.dataset.translated !== m[2]))
              queueContent(m[2], 'MED2', desc)
          }
        }
        ...
        if (settings.translateCharacters) {
          ...
          if (m) {
            ...
            const desc = document.querySelector('.description')
            if (desc && (!desc.querySelector('.ru-desc') || desc.dataset.translated !== m[1]))
              queueContent(m[1], 'CHR2', desc, m[2])
          }
        }
        ...
        if (settings.translateStaff) {
          ...
          if (m) {
            ...
            const desc = document.querySelector('.description')
            if (desc && (!desc.querySelector('.ru-desc') || desc.dataset.translated !== m[1]))
              queueContent(m[1], 'STF3', desc, m[2])
          }
        }
      }, 300)
    }
    ...
    async function processTransQueue() {
      ...
      if (pending.MED2.size > 0) {
        ...
        for (const m of res?.data?.Page?.media || []) {
          ...
          if (m.idMal) {
            ...
            if (resolved) {
              const data = {
                ru: resolved.russian,
                desc: cleanShikiBB(resolved.description, resolved.url, resolved.sourceName),
              }
        ...
        const charMap = {}
        if (res?.data?.Page?.characters)
          res.data.Page.characters.forEach((c) => (charMap[c.id] = c))
        ...
        for (const id of ids) {
          ...
          if (!shikiItem) {
            const targetMalIds = charData
              ? ((charData.media && charData.media.nodes) || []).map((n) => n.idMal).filter(Boolean)
              : []
            const sRes = await fetchShikiPersonREST(
              'characters',
              searchName,
              nativeName,
              targetMalIds,
            ...
            if (sRes.status === 200 && sRes.data) shikiItem = sRes.data
              ...
              shikiRateLimitPause = Date.now() + 6000
              pending.CHR2.set(id, fallbackName)
              break
            }
          } else {
            ...
            if (det.data)
              shikiItem = {
                ...shikiItem,
                description: det.data.description,
                url: det.data.url,
                domain: det.domain,
              }
          }
          ...
          if (shikiItem && shikiItem.russian) {
            const data = {
              ru: shikiItem.russian,
              desc: cleanShikiBB(
                shikiItem.description,
                `https://${shikiItem.domain || SHIKI_DOMAINS[0]}${shikiItem.url}`,
              ),
            }
        ...
        for (const id of ids) {
          ...
          if (!shikiItem) {
            const targetMalIds = staffData
              ? ((staffData.staffMedia && staffData.staffMedia.nodes) || [])
                  .map((n) => n.idMal)
                  .filter(Boolean)
              : []
            ...
            if (sRes.status === 200 && sRes.data) shikiItem = sRes.data
              ...
              shikiRateLimitPause = Date.now() + 6000
              pending.STF3.set(id, fallbackName)
              break
            }
          } else {
    ...
    function applyTranslation(type, id, data) {
      ...
      if (data && data.ru && data.ru !== 'NOT_FOUND') {
        items.forEach((item) => {
          ...
          if (
            item.el.classList &&
            item.el.classList.contains('title') &&
            item.el.closest('.tooltip')
          ) {
            ...
            if (!item.el.querySelector('.ru-desc')) {
              ...
              item.el.innerHTML = html`<div class="ru-desc" style="margin-bottom:20px;">
                  ${rawHTML(data.desc)}
                </div>
                <details
                  style="opacity:0.85;font-size:0.9em;background:rgba(128,128,128,0.15);padding:10px;border-radius:5px;"
                >
                  <summary style="cursor:pointer;color:#3dbbee;font-weight:bold;outline:none;">
                    Оригинальное описание (AniList)
                  </summary>
                  <div style="margin-top:10px;">${rawHTML(origHTML)}</div>
                </details>`
            }
          } else {
    ...
    const processMutations = () => {
      ...
      mutationQueue.forEach((m) => {
        if (m.addedNodes.length) {
          m.addedNodes.forEach((node) => {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
              if (node.classList && node.classList.contains('description-length-toggle'))
                node.click()
        ...
        if (m.type === 'characterData') {
          ...
          if (parent && parent.closest && parent.closest('.tooltip'))
            processTooltip(parent.closest('.tooltip'))
          changed = true
        }
        if (m.type === 'childList' && m.target.nodeType === Node.ELEMENT_NODE) {
          ...
          else if (m.target.closest && m.target.closest('.tooltip'))
            processTooltip(m.target.closest('.tooltip'))
        }
        if (
          m.type === 'attributes' &&
          ['title', 'aria-label', 'placeholder', 'value', 'label'].includes(m.attributeName)
        ) {
          translateNode(m.target)
          changed = true
        }
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      if (!rAF_ID)
        rAF_ID = requestAnimationFrame(() => {
          ...
          // Метрика перфа
          if (diff > 50)
            Logger('INFO', `[Performance] Обновление интерфейса заняло ${diff.toFixed(2)}ms`, {
  ...
  async function injectMediaExtensions() {
    ...
    if (currentMediaId !== aniId) {
      document
        .querySelectorAll(
          '.animori-ratings, .animori-franchise, .animori-themes, .animori-extlinks',
        )
    ...
    if (settings.enableFranchise && shikiData) {
      ...
      if (fRes.data && fRes.data.nodes && fRes.data.nodes.length > 1) {
        ...
        franchiseBox.classList.add('am-accent-scope')
        ...
        sorted.forEach((node) => {
          ...
          link.href = alId
            ? `/${malData.type.toLowerCase()}/${alId}`
            : `https://${usedDomain}${node.url}`
          link.className = `franchise-node ${node.id === malData.idMal ? 'active' : ''}`

          let statusText = ''
          let statusColor = ''
          let isShikiOnly = !alId
          let isCurrentPage = node.id === malData.idMal

          if (isShikiOnly) {
            statusText = ' (Только на Shiki)'
            statusColor = '#a0aec0'
            ...
            switch (listStatus) {
              case 'COMPLETED':
                statusText = isManga ? ' (Прочитано)' : ' (Просмотрено)'
                statusColor = '#a6e3a1'
                break
              case 'CURRENT':
                statusText = isManga ? ' (Читаю)' : ' (Смотрю)'
                statusColor = '#89b4fa'
                break
              case 'PLANNING':
                statusText = ' (В планах)'
                ...
                break
              case 'REPEATING':
                statusText = isManga ? ' (Перечитываю)' : ' (Пересматриваю)'
                statusColor = '#f5c2e7'
                break
              case 'PAUSED':
                statusText = ' (Отложено)'
          ...
          divYear.className = 'node-year'
          divYear.textContent = node.year || '???'
          const divTitle = document.createElement('div')
          divTitle.className = 'node-title'
          const spanTitle = document.createElement('span')
          spanTitle.textContent = node.name
          ...
          if (statusText) {
            ...
            spanStatus.style.fontSize = '0.85em'
          ...
          if (isCurrentPage) {
            ...
            spanHere.textContent = ' ⬅ Сейчас здесь'
            spanHere.style.color = 'rgb(var(--color-blue))'
            spanHere.style.fontSize = '0.85em'
            spanHere.style.fontWeight = 'bold'
            spanHere.style.marginLeft = statusText ? '4px' : '8px'
            divTitle.appendChild(spanHere)
          }

          const divKind = document.createElement('div')
          divKind.className = 'node-kind'
          divKind.textContent = node.kind
          ...
          const setExpanded = (state) => {
            ...
            if (!expanded) {
              setTimeout(() => {
                ...
                if (activeNode)
                  list.scrollTop =
                    activeNode.offsetTop - list.clientHeight / 2 + activeNode.clientHeight / 2
  ...
  window.ensureWidgets = function () {
    ...
    const path = window.location.pathname.split('/')
    if (!(path[1] === 'anime' || path[1] === 'manga') || parseInt(path[2]) !== currentMediaId)
      return
    ...
    if (
      ...
      shikiData &&
      !document.querySelector('.animori-ratings')
    ) {
      ...
      if (shikiData.rates_scores_stats) {
        let sum = 0
        shikiData.rates_scores_stats.forEach((s) => {
          sum += parseInt(s.name) * s.value
          votes += s.value
        })
      ...
      ratesBox.innerHTML = html`
        <a
          ...
          class="rating-item shiki-badge"
          ><span class="rating-star">★</span><span class="rating-label">SHIKIMORI</span
          ><span class="rating-value">${pureScore}</span></a
        >
        <a href="${malLink}" target="_blank" rel="noopener noreferrer" class="rating-item mal-badge"
          ><span class="rating-star">★</span><span class="rating-label">MYANIMELIST</span
          ><span class="rating-value">${shikiData.score || 'N/A'}</span></a
        >
        <div
          class="rating-item al-badge"
          title="Официальная средняя оценка AniList"
          style="cursor:default;"
        >
          <span class="rating-star">★</span><span class="rating-label">ANILIST</span
          ><span class="rating-value al-score-val">${alScoreText}</span>
        </div>
      `
      ...
      const buildHisto = (label, map) => {
        ...
        for (let i = 1; i <= 10; i++) {
          ...
          bar.title = `${i}: ${v.toLocaleString('ru-RU')}`
      ...
      try {
        const map = {}
        ;(shikiData.rates_scores_stats || []).forEach((x) => {
          const k = parseInt(x.name)
          if (k >= 1 && k <= 10) map[k] = x.value
        })
        ...
        const anchor =
          ratesBox.querySelector('.mal-badge') || ratesBox.querySelector('.shiki-badge')
        if (histo && anchor) {
          histo.classList.add('am-histo-shiki')
          anchor.appendChild(histo)
        }
      } catch (e) {
        Logger('WARN', 'Гистограмма Shikimori не построена', e)
      }
    }
    ...
    if (franchiseBox) {
      const existing = document.querySelector(
        '.animori-franchise:not(.animori-themes):not(.animori-extlinks)',
      )
    ...
    if (
      ...
      malData.type === 'ANIME' &&
      sidebar &&
      !document.querySelector('.animori-themes')
    ) {
      const themesBox = document.createElement('div')
      themesBox.classList.add('am-accent-scope')
      ...
      fetchMalThemes(malData.idMal).then((themes) => {
        ...
        serviceToggle.innerHTML = ['vk', 'yt', 'spotify', 'sc']
          .map(
            (v) =>
              html`<div
                class="am-service-btn ${activeMusicService === v ? 'active' : ''}"
                data-val="${v}"
                title="${svcTitles[v]}"
                aria-label="${svcTitles[v]}"
              >
                ${rawHTML(svcIcons[v])}
              </div>`,
          )
        ...
        headerFlex.appendChild(serviceToggle)

        const listEl = document.createElement('div')
        listEl.className = 'themes-list'
        ...
        const renderTrack = (track, type) => {
          ...
          const searchQ = [cleanTitle.replace(/\s*\(eps.*?\)/i, ''), artist]
            .filter(Boolean)
            .join(' ')
            .trim()
          ...
          wrap.className = 'am-theme-track'
          wrap.classList.add(type === 'OP' ? 'is-op' : 'is-ed')
        ...
        if (themes.openings.length > 0)
          themes.openings.forEach((op) => listEl.appendChild(renderTrack(op, 'OP')))
        if (themes.endings.length > 0)
          themes.endings.forEach((ed) => listEl.appendChild(renderTrack(ed, 'ED')))

        themesBox.appendChild(headerFlex)
        themesBox.appendChild(listEl)
        ...
        serviceToggle.querySelectorAll('.am-service-btn').forEach((btn) => {
          btn.onclick = () => {
            ...
            activeMusicService = btn.dataset.val
            GM_setValue('am_music_service', activeMusicService)
    ...
    // Внешние ссылки
    if (
      settings.enableExtLinks &&
      (malData.type === 'ANIME' || malData.type === 'MANGA') &&
      sidebar &&
      !document.querySelector('.animori-extlinks')
    ) {
      const extBox = document.createElement('div')
      extBox.className = 'animori-extlinks animori-franchise am-accent-scope'
      const pTitle = document.createElement('h2')
      pTitle.textContent = malData.type === 'ANIME' ? 'Где посмотреть' : 'Где почитать'
      pTitle.style.textAlign = 'center'
      pTitle.style.marginBottom = '15px'
      extBox.appendChild(pTitle)

      const pList = document.createElement('div')
      pList.style.cssText = 'display:flex; flex-direction:column; gap:8px;'

      const romaji = malData.title.romaji
      const ruTitle = shikiData?.russian || romaji
      const yummyDomain = settings.yummyDomain || 'yummyanime.tv'
      const animegoDomain = settings.animegoDomain || 'animego.org'
      const mangalibDomain = settings.mangalibDomain || 'mangalib.me'
      ...
      const createExtLink = (name, colorSpec, href, opts = {}) => {
        ...
        if (colorSpec && colorSpec.token)
          a.style.setProperty(
            '--c',
            `var(--color-${colorSpec.token}, ${tokenFallback[colorSpec.token] || '120, 130, 150'})`,
          )
        else a.style.setProperty('--c', (colorSpec && colorSpec.triple) || '120,130,150')

        const av = document.createElement('span')
        av.className = 'am-extlink-av'
        av.textContent = (name.trim()[0] || '?').toUpperCase()

        const info = document.createElement('span')
        info.className = 'am-extlink-info'
        const nm = document.createElement('span')
        nm.className = 'am-extlink-name'
        ...
        dom.className = 'am-extlink-domain'
      ...
      let linksAdded = 0
      if (settings.enableLinkRutracker) {
        pList.appendChild(
          createExtLink(
            'RuTracker',
            { token: 'orange' },
            `https://rutracker.org/forum/tracker.php?nm=${encodeURIComponent(romaji)}`,
          ),
        )
        linksAdded++
      }
        ...
        if (settings.enableLinkYummy) {
          pList.appendChild(
            createExtLink(
              'YummyAnime',
              { token: 'pink' },
              `https://${yummyDomain}/index.php?do=search&subaction=search&story=${encodeURIComponent(ruTitle)}`,
            ),
          )
          linksAdded++
        }
        if (settings.enableLinkAnimego) {
          pList.appendChild(
            createExtLink(
              'AnimeGO',
              { token: 'purple' },
              `https://${animegoDomain}/search/anime?q=${encodeURIComponent(ruTitle)}`,
            ),
          )
          linksAdded++
        }
      } else if (malData.type === 'MANGA') {
        if (settings.enableLinkMangalib) {
          pList.appendChild(
            createExtLink(
              'MangaLib',
              { token: 'blue' },
              `https://${mangalibDomain}/ru/catalog?q=${encodeURIComponent(ruTitle)}`,
            ),
          )
          linksAdded++
        }
      ...
      customLinks.forEach((cl) => {
        ...
        pList.appendChild(
          createExtLink(cl.name, { triple: cl.color || '120,130,150' }, url, { custom: true }),
        )
        linksAdded++
      })
      ...
      if (linksAdded > 0) {
        ...
        const themesBlock = sidebar.querySelector('.animori-themes')
        const ratingsBlock = sidebar.querySelector('.animori-ratings')
        if (themesBlock) themesBlock.after(extBox)
        else if (ratingsBlock) ratingsBlock.after(extBox)
        else sidebar.prepend(extBox)
      }
    }
    ...
    if (settings.enablePlayer && malData.type === 'ANIME') {
      ...
      if (!btn) {
        ...
        if (actionsContainer) {
          ...
          btn.id = 'ru-player-btn'
          btn.className = 'am-premium-btn'
          btn.innerHTML = '▶ Плеер'
          btn.title = 'Смотреть онлайн'
      ...
      if (btn) {
        ...
        if (!document.getElementById('ru-player-overlay')) {
          ...
          overlay.innerHTML = html`<div id="ru-player-shell">
            <div id="ru-stage-col">
              <div id="ru-info-panel">
                <div id="ru-title-wrap">
                  <div id="ru-title-track"><span id="info-anime-title">Загрузка...</span></div>
                </div>
                <span id="ru-ep-chip" style="display:none;"></span>
              </div>
              <div id="ru-player-container">
                <iframe id="ru-p-iframe" allowfullscreen allow="autoplay; fullscreen"></iframe>
              </div>
            </div>
            <div id="ru-sidebar">
              <div id="ru-sidebar-head">
                <span class="ru-sb-title">Озвучка</span>
                <div id="ru-player-close">&times;</div>
              </div>
              <div id="ru-translations-panel" style="display:none;"></div>
              <div id="ru-eps-label" style="display:none;">Эпизоды</div>
              <div id="ru-episodes-panel" style="display:none;"></div>
            </div>
          </div>`
          document.body.appendChild(overlay)
          const closeOverlay = () => {
            overlay.style.display = 'none'
            document.getElementById('ru-p-iframe').src = ''
          }
        ...
        btn.onclick = async () => {
          ...
          const overlay = document.getElementById('ru-player-overlay')
          overlay.style.display = 'flex'

          const rusTitle = shikiData?.russian
          const romTitle = malData.title.romaji
          const defaultTitle = rusTitle || romTitle
          ...
          function amHeartSVG(filled) {
            const c = filled
              ? 'rgb(var(--color-pink, 243,139,168))'
              : 'rgb(var(--color-text-light))'
          ...
          function amSetPlayerTitle(text) {
            ...
            requestAnimationFrame(() => {
              if (titleEl.scrollWidth > wrap.clientWidth + 4) {
                const dup = titleEl.cloneNode(true)
          ...
          amSetPlayerTitle('Подключение к базе...')

          let userProgress = 0
          let userStatus = null
          if (getAlToken()) {
            try {
              const progRes = await anilistQuery(
                `query($id:Int){Media(id:$id){mediaListEntry{progress status}}}`,
                { id: currentMediaId },
                true,
              )
              if (progRes?.data?.Media?.mediaListEntry) {
                userProgress = progRes.data.Media.mediaListEntry.progress || 0
                userStatus = progRes.data.Media.mediaListEntry.status
              }
            } catch (e) {
              Logger('ERROR', 'Ошибка получения прогресса AL для плеера', e)
            }
          }
          ...
          GM_xmlhttpRequest({
            method: 'GET',
            url: `https://kodik-api.com/search?token=${kodikToken}&shikimori_id=${malData.idMal}`,
            onload: (res) => {
              try {
                const data = JSON.parse(res.responseText)
                if (data.results && data.results.length > 0) {
                      ...
                      if (r.seasons) {
                        ...
                        if (seasonKeys.length > 0) {
                          ...
                          if (firstSeason.episodes)
                            eps = Object.keys(firstSeason.episodes)
                              .map(Number)
                              .sort((a, b) => a - b)
                        }
                      }
                  ...
                  let favs = GM_getValue('am_fav_translations', [])
                  let defaultTr = null
                  for (let fav of favs) {
                    const match = translations.find((t) => t.title === fav)
                    if (match) {
                      defaultTr = match
                      break
                    }
                  ...
                  let activeEpisode =
                    activeTranslation.episodes.length > 0 ? activeTranslation.episodes[0] : 1
                  let loadedTranslation = null // озвучка в iframe

                  const setTitle = () => {
                    amSetPlayerTitle(`${defaultTitle} — ${activeTranslation.title}`)
                    if (epChip) {
                      if (activeTranslation.type === 'anime-serial') {
                  ...
                  const updatePlayer = (seamless = false) => {
                    ...
                    if (
                      ...
                      try {
                        ...
                        Logger('ERROR', 'Kodik API change_episode', e)
                      }
                    } else {
                      iframe.src = isSerial
                        ? activeTranslation.link + '&episode=' + activeEpisode
                  ...
                  const renderEpisodes = () => {
                    ePanel.innerHTML = ''
                    if (
                      activeTranslation.type === 'anime' ||
                      activeTranslation.episodes.length <= 1
                    ) {
                      ePanel.style.display = 'none'
                  ...
                  const renderTranslations = () => {
                    ...
                    translations.forEach((tr) => {
                      ...
                      btnTr.className = `tr-btn ${tr.title === activeTranslation.title ? 'active' : ''} ${isFav ? 'favorite' : ''}`
                      const nameSpan = document.createElement('span')
                      nameSpan.className = 'tr-name'
                      nameSpan.textContent = tr.title
                      const heartSpan = document.createElement('span')
                      heartSpan.className = 'tr-heart'
                      heartSpan.innerHTML = amHeartSVG(isFav)
                      ...
                      heartSpan.onclick = (e) => {
                        ...
                        if (currentFavs.includes(tr.title))
                          currentFavs = currentFavs.filter((f) => f !== tr.title)
                        else currentFavs.unshift(tr.title)
                        GM_setValue('am_fav_translations', currentFavs)
                        favs = currentFavs
                        renderTranslations()
                      }
                      btnTr.appendChild(nameSpan)
                      btnTr.appendChild(heartSpan)
                  ...
                  if (window.__amKodikSync)
                    window.removeEventListener('message', window.__amKodikSync)
                  window.__amKodikSync = (message) => {
                    ...
                    if (!ep || ep === activeEpisode || !activeTranslation.episodes.includes(ep))
                      return
                    activeEpisode = ep
                    renderEpisodes()
                    setTitle()
                ...
                Logger('ERROR', 'Kodik API: сбой парсинга ответа search', e)
                fallbackPlayer('API Error')
              }
            },
            onerror: () => {
              fallbackPlayer('Network Error')
            },
          })
        }
      }
  ...
  function initDictCapture() {
    ...
    document.addEventListener('mouseup', (e) => {
      ...
      setTimeout(() => {
        ...
        pop.querySelector('.am-dict-cap-btn').onclick = () => {
          openForm(rect)
        }
      }, 10)
    })
    const openForm = (rect) => {
      ...
      inp.select()
      const save = () => {
        if (upsertUserDictEntry(currentSel, inp.value)) {
          removeForm()
  ...
  function initRussianSearch() {
    ...
    document.body.addEventListener('input', (e) => {
      ...
      if (target.tagName !== 'INPUT' || target.getAttribute('placeholder') !== 'Поиск в AniList')
        return
    ...
    async function performRussianSearch(query) {
      ...
      try {
        ...
        const tagDomain = (res) =>
          (res.data || []).map((i) => ({ ...i, __domain: res.domain || SHIKI_DOMAINS[0] }))
        ...
        shikiChars.forEach((c, i) => {
          ...
          rootFields.push(
            `pc${i}: Page(perPage:1){ characters(search:$c${i}){ id image{ large } } }`,
          )
          vars[`c${i}`] = c.name
        })
        ...
        const alData = alRes?.data?.pm?.media || []
        const alMap = {}
        alData.forEach((item) => {
        ...
        const generateCol = (title, items, typeStr) => {
          ...
          items.forEach((item) => {
            ...
            colHtml += html`<div class="result">
              <div>
                <a href="/${String(alItem.type).toLowerCase()}/${alItem.id}" class=""
                  ><div class="image" style="background-image: url('${coverSafe}');"></div>
                  <div class="name">
        ...
        const generatePersonCol = (title, items, aliasPrefix, listKey, alPath) => {
          ...
          items.forEach((item, i) => {
            ...
            const imgUrl =
              alId && node.image && node.image.large
                ? node.image.large
        ...
        resultHtml += generateCol('Аниме (RU)', shikiAnime, 'Anime')
        resultHtml += generateCol('Манга (RU)', shikiManga, 'Manga')
    ...
    function renderCustomResults(htmlContent) {
      ...
      if (!resultsContainer) {
        ...
        if (!resultsContainer) {
          ...
          if (inputWrap && inputWrap.parentNode) {
            ...
            const dataAttr = Array.from(inputWrap.attributes).find((a) =>
      ...
      wrapper.innerHTML = html`${rawHTML(htmlContent)}`
  ...
  async function init() {
    ...
    if (IS_SHIKI) {
      ...
      actionsRoot.id = 'animori-actions'
      actionsRoot.classList.add('am-accent-scope')
      document.body.appendChild(actionsRoot)
      const btnSet = document.createElement('button')
      btnSet.id = 'am-set-btn'
      btnSet.className = 'am-premium-btn'
      btnSet.innerHTML = '⚙'
      btnSet.title = 'Настройки AniMori'
      btnSet.onclick = () => {
        const p = document.getElementById('am-panel')
        p.style.display = window.getComputedStyle(p).display === 'none' ? 'flex' : 'none'
      }
      ...
      if (settings.enableLogger) {
        ...
        btnLog.id = 'am-log-btn'
        btnLog.className = 'am-premium-btn'
        btnLog.innerHTML = '&lt;/&gt;'
        btnLog.title = 'Открыть логгер (AniMori)'
        btnLog.onclick = openLoggerModal
        actionsRoot.appendChild(btnLog)
      }

      // Кнопка сравнения списков (сканер дельты)
      const btnCmp = document.createElement('button')
      btnCmp.id = 'am-cmp-btn'
      btnCmp.className = 'am-premium-btn'
      btnCmp.innerHTML = '⇄'
      btnCmp.title = 'Сравнить списки Shikimori и AniList (AniMori)'
      btnCmp.onclick = openCompareModal
      actionsRoot.appendChild(btnCmp)

      // sw() — доверенный HTML → rawHTML()
      const sw = (id, on, extra = '') =>
        rawHTML(
          `<label class="amk-switch"><input type="checkbox" id="${id}" ${on ? 'checked' : ''} ${extra}><span class="amk-track"></span><span class="amk-thumb"></span></label>`,
        )
        ...
        <div class="amk-modal">
          ...
          <div class="amk-body amk-tabbed">
            <nav class="amk-tabnav">
              <button type="button" class="amk-tab active" data-tab="translate">
                <span class="amk-tab-ic"
                  ...
                  >
                    ...
                    <path d="M3 12h18" />
                    <path d="M12 3a15 15 0 0 1 0 18 15 15 0 0 1 0-18" /></svg></span
                >Перевод
              </button>
              <button type="button" class="amk-tab" data-tab="dict">
                <span class="amk-tab-ic"
                  ><svg
                    viewBox="0 0 24 24"
                    width="16"
                  ...
                  >
                    ...
                    /></svg></span
                >Словарь<span class="amk-tab-count" id="am-dict-count" hidden>0</span>
              </button>
              <button type="button" class="amk-tab" data-tab="modules">
                <span class="amk-tab-ic"
                  ><svg
                    viewBox="0 0 24 24"
                    width="16"
                  ...
                  >
                    ...
                    <rect x="3" y="14" width="7" height="7" rx="1" /></svg></span
                >Модули
              </button>
              <button type="button" class="amk-tab" data-tab="appearance">
                <span class="amk-tab-ic"
                  ><svg
                    viewBox="0 0 24 24"
                    width="16"
                    ...
                    stroke-linejoin="round"
                  >
                    <path d="M12 3s6 6.5 6 10.5a6 6 0 0 1-12 0C6 9.5 12 3 12 3z" /></svg></span
                >Оформление
              </button>
              <button type="button" class="amk-tab" data-tab="links">
                <span class="amk-tab-ic"
                  ><svg
                    viewBox="0 0 24 24"
                    width="16"
                  ...
                  >
                    ...
                    <path d="M13 18l-1 1a4 4 0 0 1-6-6l2-2" /></svg></span
                >Ссылки
              </button>
              <button type="button" class="amk-tab" data-tab="account">
                <span class="amk-tab-ic"
                  ><svg
                    viewBox="0 0 24 24"
                    width="16"
                  ...
                  >
                    ...
                    <path d="M16 7l3 3" /></svg></span
                >Аккаунт
              </button>
              <button type="button" class="amk-tab" data-tab="misc">
                <span class="amk-tab-ic"
                  ><svg
                    viewBox="0 0 24 24"
                    width="16"
              ...
              <button type="button" class="amk-tab" data-tab="support">
                <span class="amk-tab-ic"
                  ><svg
                    viewBox="0 0 24 24"
                    width="16"
              ...
              <div class="amk-pane am-notr" data-pane="dict">
                ...
                <div class="amk-card">
                  ...
                  <div style="display:flex; gap:8px; flex-wrap:wrap;">
                    <button class="amk-btn amk-btn-ghost" id="am-dict-export" style="flex:1;">
                      Экспорт
                    </button>
                    <button class="amk-btn amk-btn-ghost" id="am-dict-import" style="flex:1;">
                      Импорт
                    </button>
                    <button class="amk-btn amk-btn-ghost" id="am-dict-copy" style="flex:1;">
                      Копировать
                    </button>
                  </div>
                  <button
              ...
              <div class="amk-pane" data-pane="links">
                ...
                <div class="amk-card">
                  ...
                  <div class="amk-row-hint" style="padding:10px 2px 2px; line-height:1.5;">
                    ...
                    <code
                      style="background:rgba(var(--color-text-light),0.12);padding:1px 5px;border-radius:4px;"
                      >{ru}</code
                    >
                    — русское название,
                    <code
              ...
              <div class="amk-pane" data-pane="account">
                <div class="amk-card">
                  ...
                  <div class="amk-row-hint" style="padding:8px 2px 6px;">
                    ...
                    <code
                      style="background:rgba(var(--color-text-light),0.12);padding:1px 5px;border-radius:4px;"
                      >https://anilist.co/api/v2/oauth/pin</code
                    >
                  </div>
                  <input
                    class="amk-input amk-mono"
                  ...
                  <div class="amk-row">
                    <span class="amk-row-label"
                      ...
                      ><span class="amk-row-hint"
                        >отслеживание действий скрипта (для отладки)</span
                      ></span
              ...
              <div class="amk-pane" data-pane="support">
                <div class="amk-card">
                  ...
                  <div class="amk-row-hint" style="padding:2px 2px 10px; line-height:1.55;">
                    ...
                    нужно. Если тулкит вам пригодился, лучшая благодарность — пара действий ниже.
                    Это правда помогает.
                  </div>
                  <button
                    class="amk-btn amk-btn-primary amk-btn-block"
                ...
                <div class="amk-card">
                  ...
                  <div style="display:flex; gap:8px;">
                    <input
                      ...
                      id="am-sup-link"
                      readonly
                      value="https://greasyfork.org/scripts/572948"
                      style="flex:1;"
                    />
      ...
      panel.querySelectorAll('.amk-tab').forEach((tb) => {
        tb.onclick = () => {
          ...
          panel
            .querySelectorAll('.amk-pane')
            .forEach((pn) => pn.classList.toggle('active', pn.dataset.pane === key))
        }
      })
      ...
      if (supReview)
        supReview.onclick = () => window.open(SUP_GREASY + '/feedback', '_blank', 'noopener')
      ...
      document.getElementById('set_yummy_domain').value = settings.yummyDomain
      document.getElementById('set_animego_domain').value = settings.animegoDomain
      document.getElementById('set_mangalib_domain').value = settings.mangalibDomain

      // Источники перевода тайтлов: значения + связка (фоллбэк != основной).
      {
        const tp = document.getElementById('set_title_primary'),
          tf = document.getElementById('set_title_fallback')
        if (tp && tf) {
          tp.value = settings.titlePrimary
          tf.value = settings.titleFallback
          const syncTitleSrc = () => {
            const off = tp.value === 'off'
            tf.disabled = off
            Array.from(tf.options).forEach((o) => {
              o.disabled = o.value !== 'none' && o.value === tp.value
            })
            if (off || tf.value === tp.value) tf.value = 'none'
          }
          syncTitleSrc()
          tp.onchange = () => {
            GM_setValue('set_title_primary', tp.value)
            syncTitleSrc()
            GM_setValue('set_title_fallback', tf.value)
          }
          tf.onchange = () => {
            GM_setValue('set_title_fallback', tf.value)
          }
      ...
      {
        const c = document.getElementById('am-set-close')
        if (c)
          c.onclick = () => {
      ...
      booleanSettings.forEach((id) => {
        const el = document.getElementById(id)
        if (el) el.onchange = (e) => GM_setValue(id, e.target.checked)
      })
      ...
      const renderCustomLinksEditor = () => {
        ...
        links.forEach((cl, idx) => {
          ...
          urlIn.placeholder = 'https://site.com/search?q={query}'
          urlIn.value = cl.url || ''
          urlIn.style.marginTop = '6px'
          const sw = document.createElement('div')
          sw.className = 'am-cl-swatches'
          CL_COLORS.forEach((c) => {
            const s2 = document.createElement('span')
            s2.className = 'am-cl-sw' + (cl.color === c ? ' active' : '')
          ...
          const save = () => {
            const arr = getCustomLinks()
            if (arr[idx]) {
              arr[idx].name = nameIn.value.trim()
              arr[idx].url = urlIn.value.trim()
              setCustomLinks(arr)
            }
          }
      ...
      renderCustomLinksEditor()
      {
        ...
        if (addBtn)
          addBtn.onclick = () => {
            const arr = getCustomLinks()
            arr.push({ name: '', url: '', color: CL_COLORS[arr.length % CL_COLORS.length] })
            setCustomLinks(arr)
            renderCustomLinksEditor()
          }
      }
      ...
      const renderDictEditor = () => {
        ...
        const keys = Object.keys(ud)
          ...
          .filter(
            (k) => !q || k.toLowerCase().includes(q) || String(ud[k]).toLowerCase().includes(q),
          )
        ...
        keys.forEach((k) => {
          ...
          del.title = 'Удалить'
          const commit = () => {
            const nk = normDictKey(srcIn.value),
              nv = normDictKey(trIn.value)
            if (!nk || !nv) return
              ...
              fr.onload = () => {
                try {
                  ...
                  Object.keys(obj).forEach((k) => {
                    const nk = normDictKey(k),
                      nv = normDictKey(obj[k])
                    if (nk && nv) ud[nk] = nv
                  })
                  setUserDict(ud)
                  rebuildDictionary()
                  ...
                  alert(
                    'Не удалось разобрать файл словаря (ожидается JSON вида {"English":"Русский"}).',
                  )
                }
              }
              fr.readAsText(f)
      ...
      if (tokenInput) {
        tokenInput.value = GM_getValue('AL_TOKEN', '')
        tokenInput.onchange = (e) => GM_setValue('AL_TOKEN', e.target.value.trim())
      }
      ...
      if (genBtn) {
        genBtn.onclick = () => {
          ...
          if (!cid)
            return alert('Введите Client ID (его можно создать в настройках AniList -> Developer)')
          const authLink = document.createElement('a')
          authLink.href = `https://anilist.co/api/v2/oauth/authorize?client_id=${cid}&response_type=token`
          authLink.target = '_blank'
          authLink.style.cssText =
          ...
          wrap.innerHTML = ''
          wrap.appendChild(authLink)
        }
      }

      document.getElementById('am-apply').onclick = () => {
        location.reload()
      }
      document.getElementById('am-clear').onclick = async () => {
        await clearCache()
        alert('Кэш сброшен!')
        location.reload()
      }
      ...
      if (
        ...
        GM_xmlhttpRequest({
          ...
          onload: (res) => {
            if (res.status === 200) {
              try {
                remoteDict = Object.assign(Object.create(null), JSON.parse(res.responseText))
                Logger('INFO', 'Словарь загружен и распарсен')
              } catch (e) {
                Logger('ERROR', 'Ошибка парсинга словаря', e)
              }
            }
            rebuildDictionary()
            initTranslator()
