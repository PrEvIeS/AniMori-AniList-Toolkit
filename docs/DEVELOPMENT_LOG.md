...
1. Контракт `MediaWidget.mount()` синхронный, а франшизе нужны два сетевых запроса. Поэтому в модуле живут замок `loadingAniId` и кэш готового блока: сеть опрашивается один раз на тайтл, а при каждой перерисовке React блок просто возвращается на место.
2. Исправлен тот же дефект шаблонного URL: ссылки на тайтлы, которых нет на AniList, вели в никуда. Теперь адрес собирается конкатенацией домена зеркала и пути узла, плюс добавлен `rel="noopener noreferrer"`.
...
5. Поведение с пустым результатом сохранено намеренно: блок остаётся в DOM скрытым, а не удаляется. Иначе восстановление виджетов считало бы блок потерянным и запрашивало темы по кругу.
6. После ответа сети повторно проверяется, что открыт тот же тайтл: треки одного аниме не попадут на страницу другого.
7. Обработчики кнопок повешены через `addEventListener` вместо `onclick`; кнопка копирования гасит переход по ссылке, внутри которой находится.

### `04eca18` и `de81e07` — часть 3 из 3, виджет 4: внешние ссылки

`src/features/media/extlinks.ts` — блок «Где посмотреть» для аниме и «Где почитать» для манги: карточки RuTracker, YummyAnime, AnimeGO и MangaLib с буквенной аватаркой, названием, доменом и стрелкой, плюс пользовательские ссылки из `core/custom-links.ts` с пометкой «своя».

Домены сервисов берутся из настроек (`yummyDomain`, `animegoDomain`, `mangalibDomain`) с фолбэками `yummyanime.tv`, `animego.org`, `mangalib.me`. Шаблоны своих ссылок поддерживают `{ru}`, `{romaji}` и `{query}`. Блок встаёт после музыкальных тем, иначе после рейтингов.

### Отличия от монолита — внешние ссылки

1. Исправлен тот же дефект шаблонного URL: адреса RuTracker, YummyAnime, AnimeGO и MangaLib собирались сломанным шаблоном, то есть в 1.9.1 весь блок был некликабельным. Теперь адрес собирается конкатенацией и `encodeURIComponent`.
2. Домен из настроек нормализуется: лишний `https://`, хвостовой слеш и пробелы больше не превращают ссылку в мусорный адрес.
3. Если у тайтла нет ни русского названия, ни romaji, поисковые ссылки не создаются: пустой поиск пользователю не нужен.
4. Если все сервисы выключены и своих ссылок нет, блок не создаётся вовсе, а не рисуется пустой рамкой с заголовком.
5. Цвет карточки типизирован: либо токен темы AniList с фолбэк-триплетом, либо свой триплет из настроек. Неизвестный токен больше не даёт бесцветную карточку.
6. Домен карточки берётся из настроек, а если его нет — из самого адреса через `URL`, с защитой от битого адреса.

На этом часть 3 и перенос медиа-страницы завершены.
...
Рост 22 → 23 модулей подтверждает включение виджета рейтингов.
...
Рост 23 → 24 модулей подтверждает включение виджета франшизы.
...
Рост 24 → 26 модулей и 176.41 → 189.67 kB подтверждает включение виджета музыкальных тем вместе с его зависимостью `api/animethemes.ts`.
...
- Журнал отправляется только литеральным UTF-8. Одна отправка с экранированными символами обрезалась на середине файла и потребовала полного восстановления документа.
...
- [x] часть 3, виджет 4: внешние ссылки и свои ссылки.
...
- [ ] browser smoke-tests scanner/exporter/translator/плеера/рейтингов/франшизы/тем/внешних ссылок.

## 17. Обязательные риски

Подробности: [`AUDITION.md`](./refactoring/AUDITION.md).

1. Асинхронное Tauri storage: UI ждёт `await bridge.storage.getAll()`. Касается выбора музыкального сервиса и пользовательских ссылок: они читаются синхронно через `GM_getValue`.
2. Rust HTTP не получает cookies WebView автоматически. Касается франшизы и русских названий в блоке внешних ссылок: данные берутся с Shikimori.
...
Пункт 1.11: `src/features/search/` — расширенный поиск и захват словаря (строки 3734-3934 монолита).

Включает:

- перехват пользовательских правок названий и запись их в пользовательский словарь;
- кнопки словаря в панели `#animori-actions`;
- повторный перевод страницы после правки словаря через callback из `core/dictionary.ts`.

После этого — UI настроек, который владеет панелью `#animori-actions`, и только затем полноценный `bootstrap()` с SPA lifecycle и сборщиком мусора.
