...
> Базовая проверенная ревизия: `75bbf2a`  
> Версия: `1.9.1`
...
- [ ] часть 3, виджет 3: музыкальные темы OP/ED;
...
- [ ] browser smoke-tests scanner/exporter/translator/плеера/рейтингов/франшизы.
...
Виджет 3 части 3: `src/features/media/themes.ts` — музыкальные темы OP/ED (строки 3341-3456 монолита).

Включает:

- загрузку тем и кэш IndexedDB;
- переключатель музыкального сервиса (VK, Spotify, SoundCloud, YouTube Music) с хранением выбора в `am_music_service`;
- исправление префикса кэша `THEMES_` против `THEMES2_`, из-за которого `getDbStats` всегда показывает 0 тем;
- блок скрывается, а не удаляется, если тем нет: иначе `ensureMediaWidgets()` будет запрашивать их снова и снова.

После виджета — регистрация в `main.ts`, чистые typecheck/build на машине разработчика и отдельный атомарный commit. Затем виджет 4: `extlinks.ts`.
