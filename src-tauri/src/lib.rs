...
// Пункт 5.3, часть 2: прокси для трафика самого окна. Без cfg сознательно:
// чтение настроек одинаково на всех платформах, а разница в способе применения
// спрятана внутри модуля. Так на Linux будет внятное предупреждение в журнале,
// а не молчаливо отсутствующая настройка.
mod proxy;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Пункт 5.3: прокси для трафика окна — СТРОГО до создания окна.
            //
            // Движок читает аргументы один раз, когда поднимает своё окружение, то есть на
            // первом окне. Переставишь вызов ниже build() — прокси тихо перестанет
            // применяться, и ни одной ошибки при этом не появится.
            //
