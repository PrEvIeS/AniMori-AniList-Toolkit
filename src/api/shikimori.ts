 ...
 * Исключать отпавшие нельзя: тайтл, удалё1нный на одном зеркале, жив на другом.
 */
...
export async function fetchShiki<T = unknown>(
  ...
  for (const domain of mirrorOrder()) {
    ...
    try {
      ...
      // Отметка снимается сразу: иначе после отключения туннеля каждый запрос стучался бы в мёртвый адрес.
      if (preferredDomain === domain) {
