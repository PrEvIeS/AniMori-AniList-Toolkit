...
//
// РИСК №2 из docs/DECISIONS.md: на Этапе 4 запросы к Shikimori из Rust не увидят куки
...
//
// Этап 5, итерация 5.2, две правки.
//
//   1. Появился shikimoriTrouble() — единственный ответ на вопрос «стоит ли виджету
//      писать человеку, что источник молчит». Функция живёт здесь, а не в виджетах,
//      ровно по той же причине, по которой имена источников собираются здесь: виджет
//      не должен знать ни адресов зеркал, ни того, как из адреса получается имя
//      источника в учёте. Сведение по И (беда только когда молчат ВСЕ зеркала) —
//      прикладное правило этого клиента: пока отвечает хоть один адрес, данные будут,
//      и жаловаться не на что.
//
//   2. Порядок перебора больше не жёсткий: первое зеркало, ответившее данными,
//      запоминается на сеанс и дальше пробуется первым. Раньше каждый запрос начинал
//      с первого адреса списка, и если тот недоступен, каждый из десятков запросов
//      очереди перевода сначала выжидал полный таймаут впустую. На сотне запросов
//      это минуты ожидания на ровном месте. Состав списка и трактовка кодов не меняются:
//      любое зеркало всё равно будет опробовано, если предпочтённое не дало данных.
//
 ...
 *
 * В хранилище не пишется намеренно. Доступность домена — свойство не пользователя,
 * а сети вокруг него прямо сейчас: включённый и выключенный туннель дают разные
 * ответы, и запомненный между запусками выбор становился бы вредоносным ровно в тот
 * момент, когда обстановка меняется.
 */
...
/**
 * Порядок перебора зеркал для одного запроса.
 *
 * Список всегда полный: предпочтённый адрес только переезжает в начало, остальные
 * сохраняют исходный порядок. Исключать отпавшие адреса из перебора нельзя:
 * тайтл, удалённый на одном зеркале, жив на другом, и весь смысл списка именно в этом.
 */
...
export async function fetchShiki<T = unknown>(
  ...
  for (const domain of mirrorOrder()) {
    ...
    try {
      ...
      // Предпочтённым такое зеркало не становится: связь с ним есть, но данные
      // пришли не оттуда, и выводить его вперёд значило бы добавлять по лишнему
      ...
      // Зеркало отдало данные — с него и начнём в следующий раз. Отметка ставится
      // до разбора JSON: битое тело — беда ответа, а не адреса, и при нём второе
      // зеркало со той же вероятностью отдаст такое же.
      ...
      // Предпочтённый адрес перестал отвечать — снимаем отметку сразу, а не держим
      // её до конца сеанса. Иначе после отключения туннеля каждый запрос продолжал бы
      // стучаться в уже недоступный адрес первым — тот же самый вред, ради которого
      // вся эта правка и делалась, только закреплённый нами самими.
      if (preferredDomain === domain) {
