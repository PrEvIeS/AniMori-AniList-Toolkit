...
//
// Этап 5, итерация 5.2: появился shikimoriTrouble() — единственный ответ на вопрос
// «стоит ли виджету писать человеку, что источник молчит». Функция живёт здесь, а не
// в виджетах, ровно по той же причине, по которой имена источников собираются здесь:
// виджет не должен знать ни адресов зеркал, ни того, как из адреса получается имя
// источника в учёте. Сведение по И (беда только когда молчат ВСЕ зеркала) — прикладное
// правило этого клиента: пока отвечает хоть один адрес, данные будут, и жаловаться
// не на что. Состояние ни на выбор зеркала, ни на его порядок здесь по-прежнему НЕ
// влияет: перестановка адресов по факту доступности — отдельный пункт.
//
 ...
 *
 * Ответ строится по И, а не по ИЛИ: пока отвечает хотя бы одно зеркало, перебор
 * выше отдаст данные, и говорить человеку об отказе было бы неправдой. Возвращается
 * готовый текст состояния худшего исхода первого молчащего зеркала — виджету
 * остаётся только вставить его в строку, не разбирая состояний самому.
 */
