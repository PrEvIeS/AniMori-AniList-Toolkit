// Пункт 1.4 плана: резолвер русских названий (строки 1809-1831 монолита).
//
// Вынесен отдельным файлом, а не в shikimori.ts или anime365.ts: это оркестратор,
// который зависит от обоих источников и от настроек. Держать его внутри одного из
// клиентов означало бы циклическую зависимость между api/*.
//
// Порядок источников: основной -> фоллбэк, с отбросом дублей и значений off/none.
// Настройки читаются в момент вызова, а не при импорте — см. РИСК №1 в settings.ts.
