 ...
 * приводим к общему знаменателю по худшему случаю (риск 1 из AUDITION.md).
 */
...
/** Снимок того, что случилось с прокси при запуске приложения. */
...
export interface IProxyDiagnostics {
  /**
   * Чем закончилось применение прокси при запуске. Значение неизменно
   * до перезапуска: движок читает адрес один раз.
   */
