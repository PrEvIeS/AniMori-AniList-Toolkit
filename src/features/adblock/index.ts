// Пункт 2.10 плана: блокировка рекламных блоков самого AniList.
//
// Почему это модуль юзерскрипта, а не часть Tauri-оболочки: баннеры AniList живут
// в главном фрейме, на том же домене, где работаем мы. Доступ к этому DOM у нас есть
// полный, и решение работает одинаково в браузере, в десктопной сборке и в будущем
// порте. Сетевая блокировка (WebResourceRequested в WebView2) — отдельная история
// Этапа 4 и этот модуль не отменяет.
//
// СТАТУС НА 02.08.2026. Сетевая блокировка сделана (src-tauri/src/adblock.rs) и управляется
// тем же самым тумблером «Блокировщик рекламы», что и этот модуль: для человека это одна
// функция, а не две настройки с похожими названиями. Разделение труда такое:
//   — здесь прячется разметка баннеров самого сайта;
//   — там отбиваются сетевые запросы, в том числе реклама внутри плеера, куда этот модуль
//     дотянуться не может в принципе (чужой домен).
//
// Откуда взяты селекторы. Из живого DOM, снятого с сайта 31.07.2026:
//
//   <div data-v-1a808612 data-v-b9c3d04a class="sense-wrap sense-wrap desktop above-fold">
//     <div data-v-1a808612 data-id="6213a55e..." class="vm-placement"
//          data-gamera-placement-container="__gpc-iw33r0k327">
//       <span><span style="width:728px;height:90px;...">
...
// они меняются при каждой пересборке сайта. Класс sense-wrap в снятом образце
// указан дважды — это баг вёрстки AniList, на селектор он никак не влияет.
//
// Главная осторожность — зеркальный РИСК №3 из AUDITION.md. Там мы боимся, что сайт
 ...
 *
 * Пункт 4.7: сетевой блокировщик оболочки настраивается ДО раннего выхода по выключенной
...
/**
 * Остановка модуля. Нужна для тумблера в настройках и для LifecycleManager (2.9).
