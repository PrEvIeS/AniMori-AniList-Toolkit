// Разведка сетевых источников (пункт 4.7): кто и куда ходит внутри кадра плеера.
//
// ИНСТРУМЕНТ СОПРОВОЖДЕНИЯ ДЕСКТОПНОЙ СБОРКИ. Правка 2 августа: прежняя шапка обещала
// снести модуль вместе с NET_PROBE_SCRIPT из src-tauri/src/lib.rs сразу после первой
// охоты. Решение изменено, и оба остаются в проекте насовсем: список рекламных адресов
// в src-tauri/src/adblock.rs — снимок на дату, а рекламные сетки меняют домены. Без
// разведчика обновлять этот список нечем, и правила пришлось бы писать наугад.
//
