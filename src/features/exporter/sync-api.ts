// Этап 2 п.2.7: чистая логика переноса Shikimori → AniList.
//
// Перенесено из монолитного `features/exporter/index.ts` (строки 2086-2503 версии 1.9.1)
// без изменения алгоритмов: те же эндпоинты, те же паузы, те же правила сравнения записей.
//
...
//
// РИСК №2 из docs/DECISIONS.md: в браузере списки читаются по сессионной куке и закрытый
...
/** Куда уходит текст прогресса. В юзерскрипте это подпись кнопки, как было в 1.9.1. */
...
/**
 * Счётчик неудачных мутаций за один прогон. В 1.9.1 это была модульная переменная,
 * которую обнуляли перед стартом; поведение сохранено, доступ — через функции.
 */
let syncFailures = 0
