// Этап 1 п.1.10: типы медиа-страницы (строки 3061-3733 монолита).
//
// Вынесены отдельно, чтобы виджеты (плеер, рейтинги, франшиза, темы, ссылки)
// не импортировали друг друга ради одного интерфейса.
...
/**
 * Виджет медиа-страницы.
 *
 * Вызывается многократно: при открытии страницы и каждый раз, когда AniList
 * пересобирает разметку. Обязан сам проверять, что уже вставлен, и молча
 * выходить — иначе виджеты продублируются (РИСК №3 из docs/DECISIONS.md).
 */
