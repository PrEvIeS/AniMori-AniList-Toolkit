// Этап 2 п.2.4: логика сканера дельты Shikimori ↔ AniList.
//
// Сюда переехало всё, что не является представлением: загрузка списков и избранного,
// нормализация, сверка, статистика, игнор-лист и оркестратор скана. Функции рендера
// (cmpRender, cmpRenderDiff, cmpRenderFavs, cmpRenderSummary, cmpRenderNameFavs) удалены —
// их место заняли ScannerModal.vue и ScannerDiffCategory.vue.
//
// Почему отдельный файл, а не всё в index.ts: index.ts обязан импортировать
// ScannerModal.vue, чтобы его смонтировать; компонент читает scanner-state.ts;
// состоянию нужны эти функции. Оставь логику в index.ts — получится цикл
// index.ts -> ScannerModal.vue -> scanner-state.ts -> index.ts. Та же схема, что
// у логгера и настроек, только состояние живёт в папке своей фичи.
//
// Вместе с рендером ушёл cmpEsc: интерполяцию экранирует Vue, а rawHTML в сканере
// больше не используется вообще.
//
// Отступлений от этапа 1 ровно два, оба в оркестраторе и оба утверждены
// разработчиком 30 июля 2026 года:
//   1. мягкая отмена через CancelToken — флаг проверяется на границах шагов и между
//      чанками глубокой проверки; сетевые запросы не прерываются, процесс встаёт
//      на ближайшей границе. В 1.9.1 отмены не было вовсе, кнопка просто блокировалась;
...
/**
 * Токен мягкой отмены. Объект, а не ref: compare.ts не должен зависеть от Vue —
 * на Этапе 3 этот модуль уезжает в Bridge без правок.
 */
...
/**
 * Порядок шагов и все запросы — как в cmpRunScan этапа 1. Добавлены только проверки
 * отмены на границах и счётчик шагов; ни один запрос не переставлен и не убран.
 *
 * Ошибки наружу не глушатся: их ловит scanner-state.ts и кладёт в строку статуса,
 * ровно как раньше это делал сам cmpRunScan.
 */
