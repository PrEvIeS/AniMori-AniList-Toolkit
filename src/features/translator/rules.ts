// Этап 1 п.1.9: правила перевода интерфейса AniList (строки 2525-2624 монолита).
//
// Здесь только чистые функции: на входе строка — на выходе строка или null.
// Ни DOM, ни сети, ни кэша. Так эти правила потом можно будет вызывать
// и из Vue-компонентов Этапа 2, и накрыть тестами.
//
// null означает «перевода нет, оставляем оригинал».
// Порядок проверок перенесён 1:1: правила пересекаются между собой,
// и первое совпадение решает результат. Менять порядок нельзя.
 ...
 * hasOwnProperty, а не прямое чтение: строка вида "constructor" не должна
 * вытащить служебное свойство вместо перевода.
 */
...
/**
 * Приводит ключ к виду "Jan" / "Mon" / "Winter": регэкспы регистронезависимы,
 * а таблицы в constants.ts — нет. В монолите такая строка давала "undefined"
 * прямо в интерфейсе; здесь в худшем случае останется оригинал.
 */
