<!--
  Итерация 5.1: панель «Словарь» вынесена из SettingsModal.vue без единой правки разметки.
  Причина чисто техническая: модалка доросла до 38 КБ, а вкладка там теперь ещё одна.

  Идентификаторы (am-dict-*) и стили сохранены: на них ссылается style.scss.
  Текстовые поля строк словаря по-прежнему работают через :value + @change, а не v-model:
  запись идёт по потере фокуса, иначе словарь писался бы в хранилище посимвольно.
-->
<template>
  <div class="amk-card">
