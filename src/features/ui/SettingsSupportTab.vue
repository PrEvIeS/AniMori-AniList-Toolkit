<!--
  Итерация 5.1: панель «Поддержать» вынесена из SettingsModal.vue без правки разметки.
  Идентификаторы (am-sup-*) сохранены: на них ссылается style.scss.

  Ссылки открываются через window.open, как и раньше. В десктопной сборке навигацию на
  внешние адреса перехватывает оболочка (on_navigation в lib.rs) и отдаёт системному
  браузеру, поэтому отдельная ветка на Bridge.shell здесь не нужна.
-->
<template>
  <div class="amk-card">
