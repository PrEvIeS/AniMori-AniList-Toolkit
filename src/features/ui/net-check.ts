// Итерация 5.1: ручная проверка доступности источников.
//
// Один прогон по всем адресам, которыми пользуется приложение, с записью итога в
// core/net-health.ts. Нужна для вкладки "Разработчик": пользователь видит, какой
// именно источник не отвечает, а не только общее "что-то сломалось".
//
// Важные ограничения:
// - Прогон последовательный. Параллельный залп по десятку адресов ничего не ускоряет
//   (ограничители всё равно пропускают по одному), зато смазывает замеры времени.
// - Каждый запрос сначала берёт слот у своего ограничителя из api/rate-limit.ts, и
//   только после этого начинается отсчёт времени: ожидание в очереди — не задержка сети.
// - Ни один адрес не помечен "заблокированным" заранее. Вывод делается только по факту
...
// (`anilist:graphql`, `shikimori:<домен>`, `anime365:<домен>`, `animethemes`,
// `dictionary`). При правке идентификатора в клиенте поправить и здесь.
