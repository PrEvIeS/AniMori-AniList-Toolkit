...
async function bootstrap(): Promise<void> {
  ...
  // Сторож верхнего уровня для десктопа (macOS).
  //
  // Раньше уход окна на чужой сайт отменял on_navigation в src-tauri/src/lib.rs.
  // На macOS он больше этого не делает: wry зовёт его на КАЖДЫЙ фрейм и не даёт
  // отличить главный от вложенного, а отмена по хосту убивала кадры Turnstile
  // и плеера. Проверка переехала сюда — в главный фрейм, где ей и место:
  // бандл идёт initialization_script'ом и выполняется на любой странице окна.
  //
  // Клики по внешним ссылкам сюда не доходят: их раньше перехватил initLinks().
  // Остаётся то, ради чего сторож и заводился, — переход без клика: редирект
  // сервера, location.assign, форма на чужой домен.
  if (Bridge.platform === 'tauri' && !IS_ANILIST && !IS_SHIKI) {
    const away = location.href
    Logger('WARN', `Окно ушло на чужой адрес, возвращаемся: ${away}`)
    await step('внешний адрес в браузер', () => Bridge.shell.openExternal(away))
    await step('возврат в окно', () => Bridge.shell.back())
    return
  }
